...
class LocalCouponDataSourceImpl @Inject constructor(private val couponDao: CouponDao) :
    ...
    override suspend fun insertCoupon(coupon: Coupon) = couponDao.insertCoupon(coupon.toDto())

    override suspend fun insertCoupons(coupons: List<Coupon>) = couponDao.insertCoupons(coupons.map { it.toDto() })
